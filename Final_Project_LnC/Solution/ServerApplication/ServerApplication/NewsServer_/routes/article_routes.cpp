...
// GET /articles  — optional JWT for personalized feed
Rest::Route::Result getAllArticlesHandler(const Rest::Request& req, Http::ResponseWriter response) {
    // If the user is authenticated, use their id for a personalized feed.
    int userId = -1;
    std::string role;
    if (JwtUtils::validateRequest(req, userId, role)) {
        // userId already set from token — ignore any ?userId= param for security
    } else {
        // Fall back to query param (anonymous browse)
        auto query = req.query();
        if (query.has("userId")) {
            try { userId = std::stoi(query.get("userId").value()); }
            catch (...) { userId = -1; }
        }
    }
...
Rest::Route::Result searchArticlesHandler(const Rest::Request& req, Http::ResponseWriter response) {
    ...
    std::string sortBy    = query.get("sort").value_or("");
...
Rest::Route::Result getSavedArticlesHandler(const Rest::Request& req, Http::ResponseWriter response) {
    ...
    if (!JwtUtils::requireAuth(req, response, tokenUserId, role))
        return Rest::Route::Result::Ok;

    int paramUserId = req.param(":userId").as<int>();
    // Non-admin users can only see their own saved articles
    if (role != "admin" && tokenUserId != paramUserId) {
        response.send(Http::Code::Forbidden,
                      R"({"status":"forbidden","message":"Cannot access another user's saved articles"})");
        return Rest::Route::Result::Ok;
    }
    json result = ArticleService::getSavedArticlesForUser(paramUserId);
    response.send(Http::Code::Ok, result.dump());
    return Rest::Route::Result::Ok;
}
...
Rest::Route::Result reportArticleHandler(const Rest::Request& req, Http::ResponseWriter response) {
    ...
    response.send(Http::Code::Ok, R"({"status":"ok","message":"Article reported."})");
    return Rest::Route::Result::Ok;
}

// ── Admin-only article moderation ─────────────────────────────────────────────

// POST /admin/articles/:id/hide
Rest::Route::Result hideArticleHandler(const Rest::Request& req, Http::ResponseWriter response) {
    int userId; std::string role;
    if (!JwtUtils::requireAdmin(req, response, userId, role))
        return Rest::Route::Result::Ok;

    int articleId = req.param(":id").as<int>();
    ArticleService::hideArticle(articleId);
    response.send(Http::Code::Ok, R"({"status":"ok","message":"Article hidden."})");
    return Rest::Route::Result::Ok;
}
...
Rest::Route::Result unhideArticleHandler(const Rest::Request& req, Http::ResponseWriter response) {
    ...
    response.send(Http::Code::Ok, R"({"status":"ok","message":"Article unhidden."})");
    return Rest::Route::Result::Ok;
}
