...
Rest::Route::Result loginHandler(const Rest::Request& req, Http::ResponseWriter response) {
    ...
    if (success) {
        std::string token;
        try {
            token = JwtUtils::generateToken(userId, role);
        } catch (const std::exception& e) {
            // JWT_SECRET not set — log and still return user info without token
            std::cerr << e.what() << "\n";
        }
        ...
        json res = {
            ...
            {"user_id", userId},
            {"role",    role},
            {"token",   token}   // empty string if JWT_SECRET is not configured
        };
        response.send(Http::Code::Ok, res.dump());
    } else {
        std::cout << Strings::ROUTE_LOGIN_FAILED_LOG
                  << email << ", UserId: " << userId
                  << ", Role: " << role << "\n";
...
// POST /auth/signup  (public — no token required)
